...
function drawPointAnimation(v){
	...
	for(var i=0;i<cx*2+60;i+=15){
		x.strokeStyle=`hsla(286,95%,60%,${i%60?.05:.10})`;
		X1=-60+i+v[0].f;
		Y1=0;
		X2=X1;
		Y2=cy*2;
	...
	for(var i=0;i<cy*2+60;i+=15){
		x.strokeStyle=`hsla(286,95%,60%,${.02+i%60/150})`;
		X1=0;
		Y1=-60+i;
		X2=cx*2;
		Y2=Y1;
	...
	for(var j=0;j<cx*2+60;j+=15){
		for(var i=0;i<cy*2+60;i+=15){
			...
			if(a&&a<.045){
				x.fillStyle=`hsla(286,95%,60%,${a*3})`;
				X1=-60+j+v[0].f;
				Y1=-60+i;
		...
		for(var j=0;j<v[0].p.length;++j){
			if(i!=j){
				...
				if(d<150){
					...
					x.strokeStyle=`hsla(286,100%,60%,${1 - Math.pow(d / 150, .85)})`;
					x.stroke();
				}
			}
		}
